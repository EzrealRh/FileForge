...
- 顶栏下载图标 = **「存到手机」**：把成品统一写到 `Download/文件工坊/`（分区存储写法，不用授权）。一个固定目录，文件管理器第一屏就能找到，微信/小红书的文件选择器也默认从这里翻。不再按类型散进 Pictures / Movies / Documents —— 那种「方便」其实不方便找
- 顶栏文件夹图标 = 「导出」：想放到别的文件夹时才用它，会弹系统目录选择器
- 长按卡片看详情：GIF、动图 WebP、视频在详情页里**会直接动起来**（GIF 用自家解码器逐帧放，视频循环播放带声音，点画面暂停/继续）；列表里的缩略图仍是第一帧，一屏都在动会卡
- 详情页有「位置」一行：给出这份文件当前在哪、可一键复制路径，并说清它留在应用私有目录里（不乱写系统文件夹），要拿出去用就点顶栏「存到手机」
...
- 「存到手机」写 `Download/文件工坊/` 后，文件是否真出现在文件管理器和微信/小红书的选择器里
...
- 成品统一落 `Download/文件工坊/`（顶栏「存到手机」），不再按类型散进 Pictures/Movies/Documents；API 29 以下没有这套写法，那部分系统仍只能用「导出」选文件夹
