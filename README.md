...
- 顶栏下载图标 = **「存到手机」**：把成品统一写到 `Download/文件工坊/`（分区存储写法，不用授权）。一个固定目录，文件管理器第一屏就能找到，社交 App的文件选择器也默认从这里翻。不再按类型散进 Pictures / Movies / Documents —— 那种「方便」其实不方便找
...
- 「存到手机」写 `Download/文件工坊/` 后，文件是否真出现在文件管理器和社交 App的选择器里
