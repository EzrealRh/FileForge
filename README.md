...
| 分类 | 能做 |
| --- | --- |
| PDF | 压缩（可指定目标体积）、按体积分割、拆成 N 份、按页码截取（多段/倒序）、删除页、旋转页面、合并、提取文字、**转 Word**（按字号与位置把标题层级、列表与段落还原回来）、**转网页**（同一套结构还原，落成浏览器直接打开的 HTML）、**加页码**（三种样式、四个位置、可只给部分页）、**加文字水印**（居中或平铺、深浅和倾斜可调、中文可用）、每页导出图片、图片合成 PDF、**加打开密码与权限限制**、**去掉密码出明文副本** |
...
| 数据格式 | JSON 格式化（缩进/压平、键排序、非 ASCII 转码，**数字原文不被改写**）、JSON ↔ CSV 互转（分隔符自动识别、RFC 4180 引号、引号内换行）、JSON ↔ XML 互转、**YAML ↔ JSON 与 YAML ↔ CSV、XML ⇄ YAML、CSV → XML、YAML → Excel**（类型按 YAML 1.2 核心模式判：`yes` 与 `1:30` 保持文字，不会变成 `true` 与 `90`）、**XML → CSV / Excel**（取重复出现最多的那个元素当行，列名走元素路径 `作者.姓名`，属性带 `@`，元素自己的文字进「文本」列）—— 从哪条路转过去都是同一套判断，行列与值互相对得上 |
| Office 文档 | docx / pptx 抽正文文字成 txt（表格拍平成行、修订里删掉的字不混进来、丢了什么逐条写明），也能直接印成 PDF 或**重排成一份新 .docx**；xlsx 每张表转一份 CSV —— 日期按样式认出并转成 ISO 写法，稀疏行列按引用对位；**反向也有**：CSV / JSON 写成一份真 .xlsx，格子类型只按字面判（`007`、`1.50`、15 位以上的编号保持文字）；**Word 转 Markdown / 转网页** —— 标题层级、圆点还是编号、第几层、表格与真链接都是从文件里**读**出来的，不按字号猜 |
...
| 文本 | 换编码（UTF-8 / GB18030 / GBK / Big5 / Shift_JIS / ISO-8859-1 / UTF-16），顺带统一换行风格（LF / CRLF / CR）；把 txt 印成 PDF（A4/A5/Letter，字号·行距·页边距·首行缩进·页码可调，自动断行分页）；Markdown 转 HTML（带 charset 的完整页面）与转纯文本；**CSV 转网页表格**（首行可写成 `<th>` 表头，与「网页表格转 CSV」读的是同一种表） |
...
core/   纯 Kotlin，无安卓依赖，可 JVM 单测
  ...
  core/book/   EPUB 的读入侧：包里的 container.xml → OPF 的 manifest 与 spine（**spine 才是阅读顺序**，
               ...
               core/doc/HtmlWrite.kt：Doc → HTML/XHTML 的唯一一套渲染（EPUB 章节、转网页、
               以后别处的正文都走它）—— 标签按结构拼、`&` 与 `<` 一律转义、列表层级状态挂在每次渲染上
  ...
  core/office/ OOXML（docx / xlsx / pptx）读与写：按部件名判类型、按文件自己的大纲取页序表序（不靠文件名排序）、
               ...
               读的那一侧是它的镜像：word/document.xml + styles + numbering + 关系表 → 同一棵 Doc
               （样式 id 先过 styles.xml 翻成规范名，认不出的当正文；numId 过 abstractNum 才知道这一级是圆点
               还是十进制；r:id 过关系表拿**照字面的**外部地址 —— 拿"部件在哪"那套折算会把地址折成
               `word/https://…`；一段里的字按文档顺序收，链接与修订都不许被挪位置；表头看 tblHeader）
...
tools/verify_data_matrix.py    判数据族补齐的四条边（XML ⇄ YAML、CSV → XML、YAML → Excel）与已有的边是不是
                               **同一套判断**：PyYAML 读我们写的 YAML 与 minidom 独立摊 XML 得到的树相等、
                               XML→YAML 与 XML→JSON 同一棵树、三份产物 XML 良构且只有一个根、
                               CSV→XML 的行与列名与 csv 模块独立读来料一致、YAML→Excel 的格子与
                               Python 独立读那份表一致（参照物不能是我们自己从同一棵树写的 CSV）、
                               含逗号引号换行空值的格子转 YAML 再读回来还是那些字、YAML→XML 的键与元素
                               路径一一对得上、看着像数字的写法不被改字面
                               跑法：先 `./gradlew :core:test --tests '*DataMatrixTest'` 落盘，
                               再 `python tools/verify_data_matrix.py`
tools/verify_docx_read.py      判 docx 的结构读法：同一份 .docx，**pandoc 自己读**出来的块序列 / 标题层级 /
                               列表类型与深度 / 表格行列 / 链接地址，要和我们读出来再写成的
                               Markdown 与网页一致（42 条）；修订那一份换 **ElementTree 独立读 OOXML** 判 ——
                               pandoc 的 docx 读者压根不看 w:ins / w:del（三种 --track-changes 输出一模一样），
                               拿它当裁判等于没判
tools/reverse_docx_read.py     上面那 42 条的反向验证：改坏读法 7 处（句中链接被挪到句尾、样式名一律当正文、
                               pStyle 当属性读、列表一律按圆点、链接地址不当回事、删掉的字也留下、表格整块不读），
                               每处断言红在自己那条上；**每轮先删掉上一次的产物**再重跑落盘 ——
                               不然改坏时产物没重生成，判据会拿着旧文件说"全绿"（真发生过）
                               跑法：`python tools/reverse_docx_read.py`
tools/reverse_data_matrix.py   上面那 8 条的反向验证：改坏实现 6 处（顶层数组不包根、多键对象被拆成第一个键、
                               属性丢了 @、首行当数据行、摊表少一行、该加引号的没加），每处断言红在
                               它该红的那条上，并记录是单元测试先拦还是外部判据拦的；
                               改的是 Kotlin 源文件 —— 原字节先留下，跑完写回并按 sha1 与 git status 证明还原干净
                               跑法：`python tools/reverse_data_matrix.py`
tools/verify_html_write.py     判 Doc→HTML 那套渲染器的产物：**pandoc** 读我们写的网页与它直接读同一份来料
                               逐块比（块序列、文字），并与它读"同一棵 Doc 写的 docx"比（跨格式同一套结构）；
                               Python 的 html.parser 判完整页面（DOCTYPE + 前 512 字节的 charset）与标签配平、
                               <th> 只在首行、列表真嵌套两层、链接地址原样在 href 里、像标签的那段是文字
                               跑法：先 `./gradlew :core:test --tests '*Html*'' 落盘，再 `python tools/verify_html_write.py`
tools/reverse_html_write.py    上面那 40 条的反向验证：11 个反例各只破一处（charset 没了、DOCTYPE 没了、
                               表格少闭标签、多一个没闭合的块、标题被换、少一句、列表摊平、表头变 td、
                               链接改一个字母、两层并一层、网页与 Word 结构不同），断言点到自己那条；
                               `--comply` 断言合规页被收；原字节留内存跑完写回并按 sha1 核还原
                               跑法：`python tools/reverse_html_write.py`
...
- 数据格式这一族的参照物是 **Python 的 json 与 csv**，而且**只在该信它的地方信**：Python 的 `dumps` 会把 `1.50` 改写成 `1.5`、把 `1e20` 写成 `1e+20`，"不改写数字"恰恰是这里的诉求，所以数字用文本断言钉、转义与 CSV 单元格才拿 Python 当逐字节参照。`tools/verify_data.py` 21 条判据全过。
  - 补齐 XML ⇄ YAML、CSV → XML、YAML → Excel 这四条边时被外部裁判抓到两处自己的错：
    ① `Xml.render` 遇到顶层数组**不包根元素**，两项以上就写出两个并排的根 —— 那种文件前半截能读、
      后半截被丢（ElementTree 报 junk after document element），而 CSV→XML 正好是数组形状，不修就只能出坏文件。
      现在包一层根元素，子元素名可指定（CSV 用 `row`）；
    ② 修 ① 时把"只有一个键就拆掉那层"写成了 `members.values.firstOrNull()`，于是**多键对象只剩第一个键**
      —— 自家那条"两次结果一样"的单元测试看不见，因为它比的两次都同样少（这条后来补了"每个顶层键都在"
      的断言，反例 `多键对象也被拆成第一个键` 现在红在判据 7）
  - 判据自己的参照物也要复核：`YAML→Excel` 那条原来是拿"我们从同一棵树写的 CSV"当参照 —— 摊表少一行
    这种错两边一起少，判据恒绿。改成拿 Python 独立读那份来料当参照，同一个反例当场红（`reverse_data_matrix.py`）
  - 读的一侧会去掉元素自己文字的首尾空白（块标量末尾那几个换行因此不保留）：树是稳定的，字节不是 ——
    这条写成不动点比的是树，逐字比只在没有首尾空白的夹具上做
  - Word 转 Markdown / 转网页靠的是**把结构从文件里读回来**（`core/office/DocxRead.kt`，`DocxWrite` 的镜像读法）。
    写这一圈被自己的往返测试抓到两条：**`w:pStyle` / `w:rStyle` 是"子元素带 w:val"**，当成父元素的属性读就等于
    所有标题都是正文；**run 必须按文档顺序收**，先收 `w:r` 再收 `w:hyperlink` 会让句中的链接掉到句尾
    （`一个链接在[这里]，加粗的…` 变成 `一个链接在 ，…[这里]`）
  - 判据自己也有盲区：pandoc 的 docx 读者**完全不看** `w:ins` / `w:del`（`--track-changes` 取 accept /
    reject / all 三种，输出一个字节都不差）。所以"删掉的字不许被搬回来"这条换成 ElementTree 独立读 OOXML 来判。
    反过来也一样要小心：`reverse_docx_read.py` 第一轮里"删掉的字也留下"这个反例全绿，
    是因为它只让读法下钻 `w:del`，而被删的字写在 `w:delText` 里 —— **反例本身没打中要害**，不是判据没牙

  - 抽出共用的 Doc → HTML 渲染器（`core/doc/HtmlWrite.kt`）时，抓到一条**写在转义里的数据丢失**：
    那套 `escape` 把 `
` 当"控制字符"一起丢了，结果代码块整块挤成一行、段内换行变成"第一行第二行"连排。
    这个写法是从 EPUB 写侧原样搬过来的，所以那条错也一起在电子书里待了一版 ——
    单元测试当时看不出，因为它只断言"列表开闭齐整"这类结构性质。
    补法是转义保留换行与制表符，**由外层决定怎么写**：段落里成 `<br/>`、代码块里保持原样、
    标题里折成一个空格；并且 `<br/>` 后面不再补换行，否则"写出去再读回来"多出一个空白字符
    （这条是自家读路读回自己写的表格时发现的）
...
- **真 Word 文件的结构认得回来多少**：夹具是自己按 ECMA-376 写的最小包 + 写侧产出的包；真实文件里有本地化样式名（`标题 1`）、`basedOn` 链、`smartTag` 包着的 run、多级列表混排、图文混排的锚点，认不出来的比例有多高、话术够不够直白；从手机的文件管理器挑一份 docx 走到「Word 转 Markdown」的那条界面路径也还没在真机上点过
...
- **XML 里只有元素名与文字，没有"这是列表"这件事**：约定把孩子一律收成列表，所以 `<hosts>` 出现一次与两次，读回来是同一个形状 —— 次数只在元素重复里，不在类型里（判据比的是路径集合，不是次数）
- **Word 的结构只认对得上的样式名**：`Heading 1`…`Heading 6`、`Quote`、`Source Code`、`List Paragraph` 这几个
  （写侧自己用的就是这套）；认不出的一律当正文 —— 各家写法与本地化的名字千奇百怪，
  宁可少排一级标题，也不把作者起的名字当层级结构猜
- **下划线转 Markdown 会丢**（Markdown 没这种记号）：转网页保留成 `<u>`；docx → 网页 → 读回来这条路上它还在
- **演示文稿（.pptx）的结构不读**：幻灯片的版式是另一套规矩（占位符、版式继承），没做透就不给"转 Markdown"的入口，
  它与 docx 共用的是抽文字那条，不是这条

- **顶层是列表转 XML 时要包一层根元素**（CSV→XML 用文件名当根、每项写成 `<row>`）：一份 XML 只能有一个根元素，不包就是写一份后半截会被别人丢掉的文件
- **CSV 关掉"首行当列名"时列名写成 列1、列2…**：XML 与 JSON 的对象都得有名字，那是给没名字的东西起名字，不是把用户写过的名字改掉；列名不能当标签名时直说、不改名
- **写成网页与写成电子书共用一套渲染**：所以两处对"换行"的处理一致 —— 段落里的换行写成 `<br>`。
  Markdown 稿子里那种"为了行宽折开"的软换行因此被当成硬换行显示；反过来若把它折成空格，
  网页与 Word 就会不一致。选了一致性，并在结果说明里把按哪条路排的写出来
- **格子里的 `<br>` 在「网页 → 文档树」这一路读成一个空格**（网页表格的格子本来就当一行文字），
  保留格内换行的是「网页表格转 CSV」那条路 —— 两条读路各有各的场合，不假装它们同一条
