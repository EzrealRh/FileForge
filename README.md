...
- 长按卡片看详情：GIF、动图 WebP、视频在详情页里**会直接动起来**（视频循环播放带声音，点画面暂停/继续）；列表里的缩略图仍是第一帧，一屏都在动会卡
...
- 详情页「动起来」这块（GIF/动图 WebP 走 `ImageDecoder`→`AnimatedImageDrawable`，视频走 `MediaPlayer`→`TextureView`）只能真机看：这台机器没有渲染面，编得过但看不到。Android 8.x（API 26/27）没有 `ImageDecoder`，动图预览会退回静帧，转换不受影响
