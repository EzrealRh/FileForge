...
1. 把 `app/build/outputs/apk/debug/app-debug.apk` 传到手机（微信文件、数据线、网盘都行）
...
- `./gradlew :core:test` 34 个用例全绿：页码解析（多段/开放端点/倒序/越界/坏输入）、按体积分组（含"试探次数远小于页数"的复杂度约束）、命名补零与重名、大小解析、GIF 编解码
- GIF 解码器逐像素比对 **Pillow 生成的 4 份夹具**（含交错帧、多帧、40x40x200 色），真值也来自 Pillow
- GIF 编码器输出经 **Pillow 与 omggif 两个第三方解码器**验证：mincode=2 的 6000 像素码流全部还原一致
