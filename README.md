...
| 更新 | 应用内检查最新版、直接下载安装包、拉起系统安装器；私有仓库可填只读 token（只存本机） |
...
tools/updateprobe/ 应用内更新验证台：直接拿 :core 那份真代码打线上 GitHub API，验请求头、私有仓库的报错文案、
                 选包、302 跳到 release-assets 域名后的字节是否与本机产物一致（只取前 4KB）
...
- 应用内更新用 `bash tools/updateprobe/run.sh <只读token>` 打了线上真接口，12 条断言全过：无 UA 会 403 所以头必须带、私有仓库匿名访问的 404 会被翻成"要 token"的人话、选包选到 `*-release.apk`（7.6MB）而不是 debug（25MB）、下载端点是 api 域名、**手动跟 302 到 release-assets 域名后拿到的前 4KB 与本机刚构建的 apk 逐字节一致，总体积也对**
...
- 应用内更新在真机上整条跑一遍：手机上能不能连 `api.github.com` 和 302 之后的 `release-assets.githubusercontent.com`（两个域名都得通）、MIUI 的"安装未知应用"引导能不能跳过去、装完工作台列表和 文件工坊 目录里的成品在不在
...
- 仓库是私有的，所以匿名查版本一定拿 404 —— 面板会把 GitHub 原话翻成"要么填只读 token，要么把仓库转公开"，不会假装"已是最新版本"
