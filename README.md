...
1. 从 GitHub Release 下载 `fileforge-v<版本>-release.apk`（或 `fileforge-v<版本>-debug.apk`）传到手机（微信文件、数据线、网盘都行）
...
- 顶栏分享图标：把选中的文件走系统分享面板发给微信 / 小红书 / 蓝牙 / 邮件（经 FileProvider 临时授权，对方只拿到你交出去那几个文件）；详情页里也有「分享」给单个文件
- 筛选条上的「按日期」chip：列表按 今天 / 昨天 / 本周 / 更早 分组，组头跟着滚动停在顶上
- 转换结果会作为新文件出现在同一个列表里，**可以立刻再勾选继续做下一步**（例如 HEIC → JPG → 压缩 → 合成 PDF）
- **转换一完成就自动复制一份到手机存储**：`/storage/emulated/0/文件工坊/`（能建就建顶层，个别系统只认标准目录时退回 `/storage/emulated/0/Download/文件工坊/`）。走 MediaStore 写法，不用任何授权，文件管理器和其它 App 直接看得到 —— 他要的就是别的 App 那种 `123云盘` 式顶层目录，而不是 `/data/user/0/...` 那种进不去的地址
...
| 分类 | 能做 |
| --- | --- |
| 输入输出 | 首页「加 PDF」直达 PDF；应用内相册选择器（图片/视频，网格多选）；系统文件选择器；其他 App 分享进来；成品可再分享出去（FileProvider） |
...
- 分享：微信/小红书能不能收到（FileProvider 的 authority 是 `包名.fileprovider`，只暴露 workspace 和 staging 两个目录）
- 「按日期」分组的观感（今天/昨天/本周/更早，周一为一周起点）
- 转换后自动放出去的那份：详情页「位置」显示的是 `/storage/emulated/0/文件工坊/...` 还是退回了 `Download/文件工坊/...`，文件管理器和微信/小红书能不能直接选到
